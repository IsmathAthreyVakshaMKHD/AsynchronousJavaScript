const apiBaseUrl="https://crudcrud.com/api/58963e42cdd8437ea640aba398b9fa24/votes";
...
function handleFormSubmit(event) {
    ...
    // Promise in to async function
    async function getVoteData(voteData){
        try {
            const response=await axios.post(apiBaseUrl,voteData);
...
function displayVote(voteData) {
    ...
    deleteButton.onclick = function () {
        // Using async function
            async function deleteVoteData(voteData){
                try{
                    const response=axios.delete(`${apiBaseUrl}/${voteData._id}`);
